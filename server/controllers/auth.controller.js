...
const login = async (req, res) => {
    ...
    try {
        ...
        if (
            ...
            await queryHandler('INSERT INTO usuarios_login (id_usuario, time) VALUES (?,?)', [
                usuario.id_usuario,
                tiempo,
            ]);
            return res
        ...
        await queryHandler('DELETE FROM usuarios_login WHERE id_usuario = ?', [usuario.id_usuario]);
...
const logout = async (req, res) => {
    try {
        ...
        res.cookie('jwt', '', { httpOnly: true, secure: true, sameSite: 'None', maxAge: 10 });
        res.sendStatus(204);
    } catch (error) {
