...
export const FirmaPad = ({ type, signatureRef, disabled }) => {
  ...
  let MAXRETRIES = 20;
  let TIMEOUT_LONG = 1000;
  let TIMEOUT_SHORT = 500;
  let canvas;
  let ctx;
  let formDiv;
  let intf;
  ...
  function drawButtons() {
    ...
    ctx.fillText(
      ...
      180,
      m_capability.screenHeight - 110
    );

    let campo;
    if (type === 'denunciante') {
      campo = 'Firma y DNI del Denunciante';
    }

    if (type === 'funcionario') {
      campo = 'Firma y DNI del Funcionario';
    }

    ctx.fillText(
      campo,
      m_capability.screenWidth / 2 - 70,
      m_capability.screenHeight - 90
    );
    ...
    WacomGSS.STU.isDCAReady()
      ...
      .fail(function (ex) {
        ...
        if (ex instanceof DCANotReady) {
          ...
          alert(
            'Ha ocurrido un error. Desconencte el PAD y vuelva a intentarlo.\n' +
              ex
          );
          setTimeout(close(), 0);
        }
