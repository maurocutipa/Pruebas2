...
export default function FormularioDenunciante(props) {
                ...
                getCaptcha(cuerpo).then(e => {
                    ...
                    if (e.data.data.success == true) {
                        ...
                        if (props.tipoDenuncia == 2) {
                            datosDelitosContraPersonas = { lesiones: lesiones, homicidio: homicidio, femicidio: femicidio }
                        }


                        var datDen = { ...datosDelitosContraPersonas, ...datosDenunciaIntrafamiliar, ...datosViolenciaGenero, ...props.denuncia, ...props.denuncia?.delitoSexual, ...props.denuncia?.maltratoAnimal, idTipoDenuncia: props.strDenuncia };
                        denunciaCompleta = {
                            denuncia: datDen,
                        ...
                        if (denunciaCompleta.intervinientes) {
                            denunciaCompleta.intervinientes.forEach((interviniente, index) => {
                                if (interviniente.fechaNacimiento) { //opcional
                                    denunciaCompleta.intervinientes[index].fechaNacimiento = new Date(interviniente.fechaNacimiento).toLocaleDateString('en-GB');
                                }
                            });
                        }
                        if (denunciaCompleta.denunciantes) {
                            denunciaCompleta.denunciantes.forEach((denunciante, index) => {
                                denunciaCompleta.denunciantes[index].fechaNacimiento = new Date(denunciante.fechaNacimiento).toLocaleDateString('en-GB');
                            });
                        }
                        //fin formateo de fechas
                        if (denunciaCompleta.denuncia.certezaFecha == 0) {//no conoce la fecha y hora
                        ...
                        props.fotosDni.forEach(foto => {
                            const myNewFile = new File([foto], foto.id + '_' + foto.name, { type: foto.type });
                        ...
                        axios.post("http://192.200.0.53:3000/api/denuncias/create", dataPost, { headers: { "Content-Type": "multipart/form-data" } }).then((res) => {
    ...
    const datosArray = (array) => {
        return (
            <div className='grid'>
                {(array).map((den, index) => {//props.denunciantes
                    return (
                        <div className='col-12 md:col-6 lg:col-6 xl:col-4 mb-4' key={index}>
                            <div className='border-1 h-full'>
                                <div className='mx-3'>
                                    <h3 className='underline'>Datos {den.idIntervinienteTipo === 1 ? 'Victima' : den.idIntervinienteTipo === 5 ? 'Denunciado' : den.idIntervinienteTipo === 9 && 'Testigo'}</h3>
                                    {/* <p><span className='font-bold'>Calidad: </span>{den.calidad}</p> */}
                                    <div className='grid'>
                                        <div className='col-12 grid'>
                                            <div className='col-12 lg:col-6'>
                                                <span className='font-bold'>Numero de {den.tipoIdentificacion}: </span>{den.numeroIdentificacion}
                                            </div>
                                            {den.alias != '' &&
                                                <div className='col-12 lg:col-6'>
                                        ...
                                        <div className='col-12'>
                                            <span className='font-bold'>Identidad Autopercibida: </span>{den.identidadAutopercibida}
                                        </div>
                                        <div className='col-12'><span className='font-bold'>Fecha de Nacimiento: </span>{new Date(den.fechaNacimiento).toLocaleDateString('en-GB')}</div>
                                        <div className='col-12'><span className='font-bold'>Nacionalidad: </span>{nacionalidades.filter(e => e.value == den.idPais).name}</div>
                                    ...
                                    {den.informacionAdicional != undefined && den.informacionAdicional != null && den.informacionAdicional != '' && <div className='col-12'><span className='font-bold'>Informacion Adicional: </span>{den.informacionAdicional}</div>}
    ...
    const datosViolenciaIntrafamiliar = (datosViolenciaIntrafamiliar) => {
        return (
            <div className='mb-5 border-1 border-round pl-3'>
                <div>
                    <h4>Situaciones familiares {!datosViolenciaIntrafamiliar.viveConAgresor || !datosViolenciaIntrafamiliar.medidasRestriccion || !datosViolenciaIntrafamiliar.tituloPropiedad || !datosViolenciaIntrafamiliar.situacionHacinamiento
                        ? ': ----' : null}</h4>
                    {
                        datosViolenciaIntrafamiliar.viveConAgresor || datosViolenciaIntrafamiliar.medidasRestriccion || datosViolenciaIntrafamiliar.tituloPropiedad || datosViolenciaIntrafamiliar.situacionHacinamiento ?
                            <ul>
                                <li>¿La víctima convive con el agresor?: <span className='font-bold'>{datosViolenciaIntrafamiliar.viveConAgresor}</span>  </li>
                                <li>¿Hubieron medidas de restricción previas o denuncias contra el agresor?: <span className='font-bold'>{datosViolenciaIntrafamiliar.medidasRestriccion}</span>  </li>
                                <li>¿El agresor posee título de propiedad de la vivienda en la que vive la víctima?: <span className='font-bold'>{datosViolenciaIntrafamiliar.tituloPropiedad}</span> </li>
                                <li>¿La víctima vive en situación de hacinamiento?: <span className='font-bold'>{datosViolenciaIntrafamiliar.situacionHacinamiento}</span> </li>
                            </ul>
                            :
                            null
                    }
                ...
                <div>
                    <h4>Tipos de violencias {!datosViolenciaIntrafamiliar.tiposViolencia ? ': ----' : (datosViolenciaIntrafamiliar.tiposViolencia.length == 0 && '----')}</h4>
                    <ul>
                        {!datosViolenciaIntrafamiliar.tiposViolencia ? '' : datosViolenciaIntrafamiliar.tiposViolencia.map((item) => (
                ...
                <div>
                    <h4>Conductas que posee el agresor {!datosViolenciaIntrafamiliar.perfilAgresor ? ': ----' : (datosViolenciaIntrafamiliar.perfilAgresor.length == 0 && '----')}</h4>
                    <ul>
                        {!datosViolenciaIntrafamiliar.perfilAgresor ? '' : datosViolenciaIntrafamiliar.perfilAgresor.map((item) => (
                            <li key={item.code}>{item.label}</li>
                        ))}
                    </ul>
                </div>
                <div>
                    <h4>Víctima {!datosViolenciaIntrafamiliar.victima ? ': ----' : (datosViolenciaIntrafamiliar.victima.length == 0 && '----')} </h4>
                    <ul>
                        {!datosViolenciaIntrafamiliar.victima ? '' : datosViolenciaIntrafamiliar.victima.map((item) => (
                            <li key={item.code}>{item.label}</li>
                        ))
                        }
                    </ul>
                ...
                <div>
                    <h4>Vulnerabilidades que presenta la víctima {!datosViolenciaIntrafamiliar.caracteristicasVictima ? ': ----' : (datosViolenciaIntrafamiliar.caracteristicasVictima.length == 0 && '----')}</h4>
                    <ul>
                        {!datosViolenciaIntrafamiliar.caracteristicasVictima ? '' : datosViolenciaIntrafamiliar.caracteristicasVictima.map((item) => (
                            <li key={item.code}>{item.label}</li>
                        ))}
                    </ul>
                </div>
        ...
        <>
            <div id='container' className='mt-6 px-4 md:px-6'>
                ...
                <div className='mx-2 md:mx-5 mt-3'>
                    ...
                    <div className='mb-3'>
                        ...
                        <div className='mb-4'>
                            <div className='flex justify-content-center flex-wrap'>
                                <div className='flex align-items-center justify-content-center text-lg w-full md:w-6 mb-2 md:mb-0'><span className='font-bold'>Fecha:&nbsp;</span>{new Date().toLocaleDateString() + " " + new Date().toLocaleTimeString()}</div>
                                <div className='flex align-items-center justify-content-center text-lg w-full md:w-6'><span className='font-bold'>Tipo:&nbsp;</span>{props.strDenuncia}</div>
                            </div>
                        </div>
                        ...
                        <div className='grid text-lg'>
                            ...
                            <div className='col-12'><span className='font-bold'>¿Qué Paso?:</span> {props.denuncia.descripcionQue}</div>
                            <div className='col-12 mb-3'><span className='font-bold'>¿Cómo Paso?:</span> {props.denuncia.descripcionComo}</div>
                            <div className='col-12'>
                                <span className='font-bold'>¿Dónde Paso?:</span>
                                {
                                    props.denuncia.certezaLugar == 1 ? (<>
                                        <span>&nbsp;{localidades.find(loc => loc.value == props.denuncia.idLocalidad)?.name} </span>
                                        {props.denuncia.idBarrio && (
                                            <span>, {barrios.find(bar => bar.value == props.denuncia.idBarrio)?.name}, {props.denuncia.calleHecho}, {props.denuncia.numCalle}</span>
                                        )}
                                        <div>{props.denuncia.pisoHecho && props.denuncia.pisoHecho + ", " + props.denuncia.departamentoHecho}</div>
                                    </>) : (<>
                            ...
                            <div className='col-12'><span className='font-bold'>Descripcion Adjuntos:</span> {props.denuncia.detalleAdjunto} </div>
                        </div>

                    </div>
                    <div className='mb-3'>
                        <div className=''>
                            <h2 className='text-lightblue-mpa text-3xl'>Datos Denunciantes</h2>
                        </div>
                        {/* {datosDenunciante()} */}
                    ...
                    <div className='mb-3'>
                        <div className=''>
                            <h2 className='text-lightblue-mpa text-3xl'>Datos Intervinientes</h2>
                        </div>
                    ...
                    {props.tipoDenuncia == 5 && (
                        <div className='mb-3'>
                            <div className=''>
                                <h2 className='text-lightblue-mpa text-3xl'>Datos Violencia de Genero</h2>
                            </div>
                    ...
                    {props.tipoDenuncia == 7 && (
                        <div className='mb-3'>
                            <div className=''>
                                <h2 className='text-lightblue-mpa text-3xl'>Datos Violencia Intrafamiliar</h2>
                            </div>
                    ...
                    <div className='mb-7 border-3 border-round mt-8'>
                        <div className='mx-4 px-4 py-4'>
                            <h1 className='text-center py-3 border-bottom-2 mb-6'>Términos y condiciones</h1>
                            ...
                            <p>3. Esta aplicación te acerca la posibilidad de informar hechos delictivos a la justicia y la misma deberá ser ratificada en la delegación fiscal o fiscalía mas próxima a su ubicación dentro de los 5 (cinco) días hábiles con tu DNI físico, caso contrario el ayudante fiscal o fiscal interviniente podrá archivarla sin más trámite. Consulte las delegaciones fiscales:&nbsp;
                        ...
                        <div className="grid">
                            <div className="col-12 sm:col-5 md:col-4 ">
                                <Button
                                    ...
                                    link
                                    type='button'
                                    onClick={(e) => { props.changePaso(props.paso - 1) }}
                                    hidden={denunciaCreada}
                                />
                            </div>
                            <div className="col-12 sm:col-7 md:col-8 ">
                                <Button id='btn-finalizar' type='button' icon="pi pi-chevron-right" iconPos='right' label="Finalizar Denuncia" className='w-full btn-blue-mpa py-4' onClick={e => send()}></Button>  {/* disabled={response == null || denunciaCreada} */}
                            </div>
                        </div>
                    </form>
                </div>
                ...
                {hasId == false ?
                    ...
                    <div>
                        ...
                        <div className='mx-4'>
                            ...
                            <p>Se le envió un correo electrónico a: <span className='text-bold'>{responseDenuncia.email}</span> con el comprobante.</p>
